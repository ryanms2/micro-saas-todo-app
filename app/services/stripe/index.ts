...
export const getPlansByPrice = (priceId: string): Plan => {
...
export const getUserCurrentPlan = async (userId: string) => {
  ...
  if (!user || !user.stripePriceId) {
    throw new Error('Usuário não encontrado ou preço não definido')
  }

  const plan = getPlansByPrice(user.stripePriceId)
